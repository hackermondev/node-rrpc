...
import { ChannType, IChannelCloseRequest, IChannelMessage, ICheckConnectionMessage, ICreateChannMessage, Message, MessageOPs } from "../types/messages";
...
export declare interface Channel {
    ...
    on(event: 'message', listener: (data: Buffer, message: Message, messageCount: number) => void): this;
}
...
export class Channel extends EventEmitter {
    ...
    async connect(createChann: ICreateChannMessage) {
        ...
        let c: ICheckConnectionMessage & { op: string } = { connected: this.isServer ? 'server' : 'client', waitingForOtherConnection: true, op: 'connection' };
        ...
        const messageCallback = (channel: string, raw: any) => {
            ...
            if(data.op == 'connection') {
                ...
                this.emit('message', Buffer.from(message.data, 'base64'), data, this.recievedMessagesCount);

                if(!this.isServer && this.type == 'oneway' && this.state == ChannelState.FullyConnected && this.recievedMessagesCount == 1) {
    ...
    close() {
        ...
        const message: IChannelCloseRequest & { op: MessageOPs } = { op: 'channclosereq', to: this.isServer ? 'client' : 'server' };
    ...
    send(raw: Buffer | string | any) {
        ...
        const message: IChannelMessage & { op: MessageOPs } = { op: 'connmessage', createdAt: new Date().getTime(), data: data.toString('base64'), id: '', to: this.isServer ? 'client' : 'server' };
        return this.base.redis2.publish(this.redisPubName, this.base.parseOutgoingMessage(message));    
    }
};